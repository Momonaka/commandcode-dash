...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Modalities every Command Code route accepts by default.
     *
     * The Provider API reports no modalities, and pi-ai ships no catalog for
     * this provider, so without a route-level default every model here resolves
     * to `["text"]` — the schema materializes that value on read — and the
     * harness refuses an attached image before the request is even built.
     * Declaring the pair once on the route is the smallest claim that makes the
     * vision-capable models on it usable; a model the gateway cannot actually
     * see then fails upstream, mid-turn, instead of at attach time.
     */
    const ROUTE_INPUT = ['text', 'image']
    ...
    const DICT = {
      en: {
        ...
        signOutTitle: 'Sign out of Command Code?',
        signOutConfirm:
          'This forgets the stored API key and returns the section to its setup card. The model provider routes stay in your settings.',
        cancel: 'Cancel',
        retry: 'Retry',
        loading: 'Loading…',
        active: 'active',
      ...
      zh: {
        ...
        signOut: '退出账号',
        signingOut: '退出中…',
        signOutFailed: '退出账号失败',
        signOutTitle: '退出 Command Code 账号？',
        signOutConfirm: '将清除已保存的 API key，本分区会回到引导卡片。模型提供方路由会保留在设置里。',
        cancel: '取消',
        retry: '重试',
        loading: '加载中…',
        active: '生效中',
    ...
    /** @returns a US-dollar amount, or a bare em dash when there is no value. */
    function money(value, digits = 2) {
      const shown = num(value, digits)
    ...
    /**
     * The settled answer for each plugin route, keyed by path.
     *
     * The settings panel is unmounted whenever the user leaves it and rebuilt on
     * every return, and each rebuild used to refetch both routes — a visible wait
     * for data that had not changed. Remembering the first settled answer lets a
     * later visit render immediately; the Refresh controls call `load` directly
     * and so always go back to the network and replace what is cached here.
     *
     * The cache lives in the bundle's closure, so a full page reload starts over
     * with a fresh first load.
     */
     ...
     *
     * Only the first mount of a path fetches; every later mount renders the
     * cached answer instead, so opening the panel again is instant. An explicit
     * reload always refetches and refreshes the cache.
     *
    ...
    function useApi(path, enabled) {
      ...
      const load = useCallback(() => {
        ...
        fetch(path, { credentials: 'include', headers: { accept: 'application/json' } })
          ...
          .catch((error) => {
            if (mine !== sequence.current) return
            const failed = { status: 'error', code: 'NETWORK', message: String((error && error.message) || error) }
            responseCache.set(path, failed)
            setState(failed)
          })
      }, [path, enabled])
      useEffect(() => {
        // A path that already answered — in this mount or an earlier one — has
        // nothing left to fetch on arrival. Healthy data renders straight away,
        // and a failure keeps its card (and its Retry action) instead of showing
        // the loading skeleton again. The Refresh buttons call `load` directly,
        // which is the one way back to the network.
        if (responseCache.has(path)) return undefined
        load()
        return undefined
      }, [load, path])
      return [state, load]
    ...
    /**
     * Whether one route already declares image input.
     *
     * The read side hands back a resolved section, so the schema's `["text"]`
     * default is present even when nothing was ever written. A route with no
     * `image` entry has therefore either never been touched or was deliberately
     * pinned to text; both are answered by writing the pair.
     *
     * @param provider - one route's resolved config, if it exists.
     * @returns true when the route names `image` among its default modalities.
     */
    function acceptsImages(provider) {
      const declared = provider && provider.defaultInput
    ...
    function WindowRow(props) {
      ...
      return h(
        ...
        h(
          ...
          h('span', { className: 'ccx-windowValue' }, `${money(used)} / ${money(cap, 0)}`),
        ),
        h(
          'div',
    ...
    /**
     * Run one credential-store write and reject when the store refuses it.
     *
     * The browser's `remote` namespaces answer a `RemoteResult` — `{ ok: true }`
     * or `{ ok: false, error }` — instead of rejecting, so a plain `.then` would
     * read a refusal as a success. Normalizing the result to a rejection here
     * gives the key form and the sign-out control one failure path; a transport
     * that answers `undefined` (as the offline fakes do) still counts as stored.
     *
     * @param credentials - the credentials Remote face.
     * @param write - thunk performing the write.
     * @returns a promise that settles when the store accepts the write.
     */
    function writeCredential(credentials, write) {
      return Promise.resolve(write()).then((result) => {
        if (result !== undefined && result !== null && result.ok === false) {
          throw new Error((result.error && result.error.message) || 'the credential store refused the write')
        }
      })
    }
    ...
    function KeyCard(props) {
      ...
      const submit = useCallback(
        (event) => {
          ...
          setBusy(true)
          setError(undefined)
          writeCredential(credentials, () => credentials.set(KEY_REF, trimmed))
            .then(() => {
              setValue('')
              setBusy(false)
              props.onSaved()
      ...
      return h(
        ...
        h(
          ...
          h(
            'div',
            { className: 'ccx-grid3' },
            h(Stat, { label: t.monthly, value: money(credits.monthlyCredits) }),
            h(Stat, { label: t.purchased, value: money(credits.purchasedCredits) }),
            h(Stat, { label: t.free, value: money(credits.freeCredits) }),
          ),
        ),
        h(
          'div',
        ...
        h(
          ...
          h(
            ...
            h(Stat, { label: t.creditsUsed, value: money(period.totalCredits) }),
    ...
    /**
     * The confirmation layer standing between the sign-out control and the write.
     *
     * Signing out cannot be undone from the panel — the key is write-only, so a
     * mis-click would cost the user a trip to Studio to paste it again — and it
     * is a control sitting at the very bottom of a long scroll, which is exactly
     * where a stray click lands. The mask, Escape, and Cancel all dismiss it;
     * only the confirm button runs the write, and the dialog stays put while that
     * write is in flight so a slow or refused store cannot be misread as a
     * completed sign-out.
     *
     * Escape is taken on `window` in the capture phase and stopped there. This
     * layer lives inside the settings panel, which dismisses itself on Escape, so
     * without that the user would lose the dialog and the panel together.
     */
    function ConfirmSignOut(props) {
      ...
      useEffect(() => {
        if (cancelButton.current !== null) cancelButton.current.focus()
      }, [])
      return h(
        'div',
        { className: 'ccx-overlay', role: 'presentation' },
        h('div', { className: 'ccx-mask', 'aria-hidden': 'true', onClick: onCancel }),
        h(
          'div',
          { className: 'ccx-dialog', role: 'dialog', 'aria-modal': 'true', 'aria-labelledby': 'ccx-signout-title' },
          h('div', { className: 'ccx-dialogTitle', id: 'ccx-signout-title' }, t.signOutTitle),
          h('p', { className: 'ccx-dialogBody' }, t.signOutConfirm),
          props.error === undefined ? null : h('p', { className: 'ccx-error' }, props.error),
          h(
            'div',
            { className: 'ccx-dialogActions' },
            h(
              'button',
              { ref: cancelButton, className: 'ccx-button', type: 'button', onClick: onCancel, disabled: props.busy },
              t.cancel,
            ),
            h(
              'button',
              { className: 'ccx-button ccx-buttonDanger', type: 'button', onClick: props.onConfirm, disabled: props.busy },
              props.busy ? t.signingOut : t.signOut,
            ),
          ),
        ),
      )
    ...
    /**
     * The text-only sign-out control that closes the section.
     *
     * Signing out is a credential-store write, not a plugin-route call: the key
     * never reaches this half, so the only thing the browser can ask for is that
     * the reference be forgotten. Because the host resolves the key per request,
     * that alone is what puts the section back on the setup card — the handler
     * only has to re-read what is now true.
     *
     * The `llm-pi-ai` routes are deliberately left alone. They are configuration
     * the user also hand-edits, and forgetting a key is not the same act as
     * tearing down the provider they may still want configured.
     */
    function SignOut(props) {
      ...
      const cancel = useCallback(() => {
        // A write already in flight owns the outcome; letting the layer close
        // under it would report a sign-out that has not happened yet.
        if (busy) return
        setOpen(false)
        setError(undefined)
      }, [busy])
      const confirm = useCallback(() => {
        ...
        setBusy(true)
        setError(undefined)
        writeCredential(credentials, () => credentials.unset(KEY_REF))
          .then(() => {
            setBusy(false)
            setOpen(false)
            props.onSignedOut()
          ...
          .catch((err) => {
            setBusy(false)
            setError(`${t.signOutFailed}: ${String((err && err.message) || err)}`)
          })
      }, [props, t])
      return h(
        'div',
        { className: 'ccx-signOut' },
        h('button', { className: 'ccx-link', type: 'button', onClick: () => setOpen(true) }, t.signOut),
        open ? h(ConfirmSignOut, { t, busy, error, onCancel: cancel, onConfirm: confirm }) : null,
      )
    }
    ...
    function ModelsPanel(props) {
      ...
      const allModels = plans.flatMap((plan) => plan.next.map((model) => ({ ...model, route: plan.route })))
      ...
      return h(
        ...
        h(
          ...
          // No actions here: the account header's refresh already reloads both
          // routes, so a second control two sections down was the same button
          // twice, with the crowded look that implies this list has its own
          // source of truth.
        ),
        error === undefined ? null : h('p', { className: 'ccx-error' }, error),
    ...
    function CommandCodeSection() {
      ...
      // A key is configured exactly when the account route answered something
      // other than NO_KEY — including its failure codes, because a key that was
      // rejected is the very thing a user wants to clear before pasting a fresh
      // one. The setup card *is* the signed-out page, so there is nothing to
      // sign out of there, and a route whose transport failed told us nothing
      // about whether a key exists, so it offers no control either.
      const signedIn = Boolean(body && (body.ok || body.code !== 'NO_KEY'))
      ...
      //
      // A route that already exists keeps its catalog and its hand-edited entries:
      // the one thing it may still lack is the route-level modality default, which
      // earlier versions of this plugin never wrote. That gap is what used to make
      // every Command Code model refuse an attached image, so such a route is
      // topped up with that single field rather than rewritten.
      const catalog =
        modelsState.status === 'ready' && modelsState.body && modelsState.body.ok
          ? modelsState.body.models || []
          : undefined
      const providers = (section && section.providers) || {}
      // Each pending item is `route:<key>` to write a whole missing route, or
      // `input:<key>` to add just the modality default. Encoding the decision in
      // the key keeps the effect's dependencies scalar, so it re-runs when the
      // plan changes rather than on every render.
      const todo =
        snapshot && snapshot.status === 'ready' && catalog !== undefined && catalog.length > 0
          ? ROUTES.map((route) => {
              if (providers[route] === undefined) return `route:${route}`
              return acceptsImages(providers[route]) ? undefined : `input:${route}`
            }).filter((item) => item !== undefined)
          : []
      const [provision, setProvision] = useState({ status: 'idle' })
      const attempted = useRef(false)
      const todoKey = todo.join('|')

      useEffect(() => {
        if (attempted.current || todoKey === '' || readOnly) return undefined
        ...
        const ops = todoKey
          ...
          .map((item) => {
            ...
            const route = item.slice(at + 1)
            if (item.startsWith('input:')) {
              return { op: 'set', path: ['providers', route, 'defaultInput'], value: [...ROUTE_INPUT] }
            }
            return planned[route] === undefined
              ? undefined
              : { op: 'set', path: ['providers', route], value: planned[route] }
          })
          .filter((op) => op !== undefined)
        if (ops.length === 0) return undefined
        attempted.current = true
        const mode = todoKey.split('|').some((item) => item.startsWith('route:')) ? 'routes' : 'input'
        setProvision({ status: 'working', mode })
        let alive = true
        Promise.resolve(scope.mutate(ops, snapshot ? snapshot.revision : undefined))
          .then(() => {
            if (!alive) return
            const added = ops.filter((op) => op.path.length === 2)
            setProvision({
              status: 'done',
              mode,
              routes: ops.map((op) => op.path[1]),
              count: added.reduce((total, op) => total + op.value.models.length, 0),
            })
          })
          .catch((error) => {
            if (!alive) return
      ...
      }, [todoKey, readOnly, catalog, snapshot])
      ...
      /**
       * Return the section to the page a first-ever visitor sees.
       *
       * The credential write itself already happened; what is left is making the
       * panel agree. Re-reading the account route is the whole transition — `load`
       * always goes to the network, so the cached signed-in body cannot replay
       * over the answer the store now gives.
       *
       * The provisioning report is dropped with it, because it would otherwise
       * float over that first-run page. The routes it wrote stay in `llm-pi-ai`:
       * the next key that checks out finds nothing left to do, or re-fills a
       * route the user has since deleted, which is why the attempt is un-armed
      ...
      return h(
        ...
        // A route that only needed its modality default is topped up silently: it
        // changes nothing the user can see, so a banner for it would be noise. A
        // refused write is still worth reporting, whichever plan produced it.
        provision.status === 'idle' || (provision.mode === 'input' && provision.status !== 'failed')
          ? null
        ...
        ok
          ...
          : null,
        // Last child on purpose: signing out ends the section.
        signedIn ? h(SignOut, { t, credentials: credentialsRef.current, onSignedOut: signOut }) : null,
      )
    }
    ...
    /**
     * The settings-nav row for this section: the plugin's own glyph and the
     * product name.
     *
     * The shell chooses a nav glyph from the section id — `models`,
     * `agent-presets`, `plugins`, and a gear for anything else — and a
     * `settings.section` registration carries no icon option, so an id this
     * shell does not know can only ever draw that gear. Its one verbatim seat is
     * the registered label, which the shell renders as the row's text, so the
     * glyph rides here instead and the stylesheet retires the fallback beside it.
     *
     * The path is inlined rather than shipped as a file: the published tarball is
     * `lib/*.js` plus the patch, and the browser half is not built, so nothing
     * carries an `.svg` to the page. `currentColor` replaces the source file's
     * fixed `#444444` so the glyph follows the nav's own text colour in either
     * theme.
     */
    function settingsNavLabel() {
      return h(
        'span',
        { className: 'ccx-navItem' },
        h(
          'svg',
          {
            className: 'ccx-navIcon',
          ...
          h('path', {
            fill: 'currentColor',
            d: 'M408.533333 298.666667v84.266666H324.266667A84.266667 84.266667 0 1 1 408.533333 298.666667z m87.466667 84.266666V298.666667A171.733333 171.733333 0 1 0 324.266667 470.4h84.266666v83.2H324.266667A171.733333 171.733333 0 1 0 496 725.333333v-84.266666h83.2V725.333333a171.733333 171.733333 0 1 0 171.733333-171.733333h-84.266666v-83.2H750.933333A171.733333 171.733333 0 1 0 579.2 298.666667v84.266666h-83.2z m83.2 170.666667h-83.2v-83.2h83.2v83.2zM324.266667 641.066667h84.266666V725.333333A84.266667 84.266667 0 1 1 324.266667 641.066667z m426.666666-258.133334h-84.266666V298.666667A84.266667 84.266667 0 1 1 750.933333 382.933333zM666.666667 725.333333v-84.266666H750.933333A84.266667 84.266667 0 1 1 666.666667 725.333333z',
          }),
        ),
        h('span', { className: 'ccx-navText' }, 'Command Code'),
      )
    }
...
.ccx-buttonDanger{border-color:transparent;background:#e5534b;color:#fff;font-weight:600}
.ccx-buttonDanger:hover:not(:disabled){opacity:.9;background:#e5534b}
.ccx-link{padding:0;border:none;border-radius:6px;background:none;color:inherit;font:inherit;font-size:12px;line-height:20px;opacity:.62;cursor:pointer;text-decoration:underline;text-underline-offset:3px}
.ccx-link:hover:not(:disabled){opacity:1}
.ccx-link:disabled{opacity:.4;cursor:default;text-decoration:none}
.ccx-signOut{display:flex;flex-direction:column;align-items:center;gap:6px;padding-top:2px;text-align:center}
.ccx-overlay{position:fixed;inset:0;z-index:1000;display:flex;align-items:center;justify-content:center;padding:24px}
.ccx-mask{position:absolute;inset:0;background:var(--dsw-alias-bg-mask-1,rgba(0,0,0,.45))}
.ccx-dialog{position:relative;z-index:1;box-sizing:border-box;display:flex;flex-direction:column;gap:10px;width:360px;max-width:100%;padding:18px 20px;border-radius:16px;border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.22));background:var(--dsw-alias-bg-layer-2,#1e1e1e);box-shadow:var(--dsw-elevation-prominent,0 12px 32px rgba(0,0,0,.35));text-align:left}
.ccx-dialogTitle{font-size:14px;font-weight:600;line-height:22px}
.ccx-dialogBody{margin:0;font-size:12px;line-height:18px;opacity:.72}
.ccx-dialogActions{display:flex;justify-content:flex-end;gap:8px;margin-top:4px}
...
.ccx-navItem{display:inline-flex;align-items:center;gap:8px;min-width:0}
.ccx-navText{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.ccx-navIcon{flex:none;width:16px;height:16px}
...
button:has(.ccx-navItem)>svg{display:none}
`
