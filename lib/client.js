...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** pi-ai settings namespace and the routes this section can sync. */
    const PI_AI_NS = 'llm-pi-ai'
    const ROUTES = ['commandcode', 'commandcode-anthropic']
    ...
    const DICT = {
      en: {
        ...
        noRouteHint: 'Add one in Settings → Models, then come back to sync its catalog.',
      ...
      zh: {
        ...
        noRouteHint: '请先到「设置 → 模型」添加，然后回来同步模型表。',
    ...
    function ModelsPanel(props) {
      ...
      const routes = useMemo(() => {
        ...
        return ROUTES.filter((route) => providers[route] !== undefined).map((route) => ({
          route,
...
`
    ...
    /** Test-only handle on the pure merge helpers; ignored by the plugin loader. */
    exports.__internals = { mergeModels, belongsOnRoute }
